class Questions {
    ...
    public int cherryPickup_memo(int r1, int c1, int r2, int c2, int n, int[][] grid, int[][][][] dp){
        if(r1 == n-1 && c1 == n-1){
        ...
        for(int[] dir: dirs){
            ...
            if(nextR1 < n && nextR2 < n && nextC1 < n && nextC2 < n && grid[nextR1][nextC1] != -1 && grid[nextR2][nextC2] != -1){
                nextCherries = Math.max(nextCherries, cherryPickup_memo(nextR1, nextC1, nextR2, nextC2, n, grid, dp));
            }
        }

        return dp[r1][c1][r2][c2] = currCherries + nextCherries;
    }
    ...
    public int cherryPickup(int[][] grid) {
        ...
        // // in case there is no path possible, answer will be -inf, we return 0
        // return Math.max(cherryPickup_memo(0,0,0,0,n,grid,dp),0); 
